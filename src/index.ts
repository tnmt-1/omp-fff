/**
 * omp-fff: FFF-powered file search extension for oh-my-pi
 *
 * Port of @ff-labs/pi-fff (v0.10.5), adapted for the omp extension runtime
 * and committed to omp only. Registers FFF-backed `ffgrep` / `fffind` tools
 * (replacing built-in `find` and `grep`) and adds FFF-backed @-mention
 * autocomplete suggestions to the interactive editor.
 *
 * omp loads this module through its legacy-pi compat layer: the
 * `@oh-my-pi/*` and `@sinclair/typebox` imports below are rewritten to the
 * host-bundled packages, while `@ff-labs/fff-bun` (the native FFI binding)
 * resolves from this package's own node_modules.
 */
...
// omp's AutocompleteProvider.getSuggestions takes (lines, cursorLine,
// cursorCol) — no abort-signal options like upstream pi. The wrapper in
// registerAutocompleteProvider contains any failure and delegates to the
// built-in provider, so the missing signal is not observable.
function createFffMentionProvider(
  getItems: (query: string) => Promise<AutocompleteItem[]>,
...
export default function fffExtension(pi: ExtensionAPI) {
  ...
  function trackHomeScanStatus(): void {
    ...
    const tick = () => {
      // omp treats an uncaught throw in a raw timer as fatal (session teardown),
      // so contain any native-layer failure here.
      try {
  ...
  async function getMentionItems(query: string): Promise<AutocompleteItem[]> {
  ...
  // omp renderer contract: renderResult(result, options, theme). The pi host
  // passes a render context with `lastComponent`; omp passes the tool args and
  // expects a fresh component each call, so we always build a new Text.
  ...
  pi.registerTool({
    ...
    renderResult(result, options, theme) {
      return renderTextResult(result, options, theme, 15);
    },
  });
  ...
  pi.registerTool({
    ...
    renderResult(result, options, theme) {
      return renderTextResult(result, options, theme, 20);
    },
  });
